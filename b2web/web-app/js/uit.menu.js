 ...
 $.widget("ui.menu", {
	...
	_create : function(){
		...
		this._provider2obj(options.dataProvider);
	...
	_provider2obj : function(data){
		if(typeof data == "array" || data instanceof Array){
			for(var i=0;i<data.length;i++){
				this._provider2obj(data[i]);
			}
		}
		else{
			if("id" in data){
				this.menuitems[data.id] = data;
				if(data.children){	
					this._provider2obj(data.children);
				}
			}
		}
	},
	...
	setDisabled : function(menuItemId,disabled){
		this.menuitems[menuItemId].disabled = disabled;
