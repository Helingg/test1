...
$.getContextPath = function(){
	/**
	 * ip address such as :
	 * http://192.168.1.1/framework/common/ui/jquery.js
	 * http://192.168.1.1/common/ui/jquery.js
	 * http://www.user-it.com/framework/common/ui/jquery.js
	 * http://www.user-it.com/common/ui/jquery.js
	 */
	...
	for(var i=0,script,src,matcher;script=scripts[i];i++){
		if((src = script.src) && (matcher = src.match(/\/jquery[^\/]*\.js$/i))){
			...
			if(!/(^\/)|(^http)/.test(script.getAttribute("src"))){
				...
				contextPath = pathname.substring(0,pathname.indexOf("/",1));
				if(src.indexOf(contextPath + matcher[0])<0){
					contextPath = "";
				}
			}
...
$.mask = new function(){
	...
	this.showPageMask = function(zIndex){
		pageMask = pageMask || $("<div class='uit-pagemask'></div>").appendTo('body'); 
	...
	this.showAjaxMask = function(){
		if(!ajaxMask){
			var html = ["<div class='uit-ajaxmask'>"];
	...
	this.showBoxLoading = function(box){
		if(!boxLoading){
			boxLoading = $("<div class='uit-boxloading'></div>");
		}
